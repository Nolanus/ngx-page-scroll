export class Util {
  ...
  /**
   * Cypress-compatible closeness assertion
   * Checks if actual is close to expected within the precision
   */
  public static expectToBeCloseTo(actual: number, expected: number, precision: number = 2) {
    const power = Math.pow(10, -precision + 1) / 2;
    const delta = Math.abs(expected - actual);
    expect(delta).to.be.lessThan(power);
  }
}
